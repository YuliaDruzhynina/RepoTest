...
from fastapi import APIRouter, File, UploadFile, status, Depends, Query
from typing import List, Optional
from sqlalchemy.ext.asyncio import AsyncSession

from app.services.image_service import upload_image as ui
...
async def upload_image(description: str,
    ...
    return await ui(description, file, tags, db, current_user)

@router.delete("/delete_image/{image_id}/", status_code=status.HTTP_204_NO_CONTENT)
async def delete_image(image_id: int, db: AsyncSession = Depends(get_conn_db), current_user: User = Depends(auth_serv.get_current_user)):
...
async def update_image_description(image_id: int, description: str, db: AsyncSession = Depends(get_conn_db), current_user: User = Depends(auth_serv.get_current_user)):
